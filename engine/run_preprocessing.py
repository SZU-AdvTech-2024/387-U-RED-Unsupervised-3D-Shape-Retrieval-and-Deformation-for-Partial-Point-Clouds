...
def collect_leaf_nodes(structurenet_json):
    ...
    # Add children to the queue recursively.
    while queue:
        node = queue.pop()
...
def load_meshes(nodes, in_dir):
    for node in nodes:
        ...
        node['vertices'] = normalize_pts(node['vertices'])
    return nodes
...
def get_semantics(nodes):
    ...
    for node in nodes:
        ...
        ## Dummy --> this was not used!!! each part-> base/seat/back
...
def sample_all_node_points(nodes, n_sample_points, dense=False, target_only=False):
    ...
    while len(np.unique(labels)) != n_nodes:
        labels = np.random.choice(n_nodes, n_sample_points, p=probs)
    ...
    for i in range(n_nodes):
        num = labels.tolist().count(i)
        node = nodes[i]
        
        node['all_points_mat'] = sample_points(node['vertices'], node['faces'],
                node['vertices_mat'], num)
        
        x0 = node['default_param']
        PA = node['all_points_mat']
        node['all_points'] = np.reshape(np.matmul(PA, x0),
        (num, 3), order='C')
                    
        n_node_sample_points = 1024
...
def merge_nodes(nodes, model_id, target_only = False):
    ...
    shape['points'], _, shape['point_labels'], _ = merge_meshes(
            ...
            np.arange(len(nodes)))
    shape['all_points'], _, shape['all_point_labels'], _ = merge_meshes(
            [node['all_points'] for node in nodes],
            [np.empty((0, 3), dtype=int) for node in nodes],
            np.arange(len(nodes)))
    ...
    ### 
    # try:
    shape['point_semantic'] = np.array([nodes[int(label)]['semantic_label'] \
            for label in shape['all_point_labels']])
    # except:
    #     print(model_id, "*"*20, shape['all_point_labels'][0])
    #     raise
...
def save_shape_data(leaves, shape, out_dir, model_id, target_only=False):
    ...
    if not target_only:
        select_keys = ['vertex_labels', 'face_labels', 'point_labels', 'faces',
                ...
                'point_semantic', 'vertex_semantic', 'all_points', 'all_point_labels']
...
def save_part_data(leaves, shape, out_dir, model_id, target_only=False):
    if not os.path.exists(out_dir): os.makedirs(out_dir)
    ...
    for node in leaves:
        ### Mesh ###
        # Save mesh.
        out_mesh_file = os.path.join(out_dir, "mesh", str(model_id) + '_'+ str(i) +'_mesh.obj')
        mesh = trimesh.Trimesh(vertices=node['vertices'], faces=node['faces'])  
        mesh.export(out_mesh_file, os.path.splitext(out_mesh_file)[1][1:])
        print("Saved '{}'.".format(out_mesh_file))

        ### Point Cloud ###
        # Save point cloud.
        out_point_cloud_file = os.path.join(out_dir, "point_cloud", str(model_id) + '_'+ str(i) + '_points.xyz')
        np.savetxt(out_point_cloud_file, node['points'], delimiter=' ', fmt='%f')
        print("Saved '{}'.".format(out_point_cloud_file))
        ...
        select_keys = ['faces', 'vertices', 'vertices_mat', 'points', 'objs', 'default_param', 'points_mat']

        out_h5_file = os.path.join(out_dir, "h5", str(model_id) + '_'+  str(i) +'_leaves.h5')
        dt = h5py.special_dtype(vlen=str)
        with h5py.File(out_h5_file, 'w') as f:
            f.create_dataset('box_params', data=box_params, compression="gzip")
            f.create_dataset('label', data=label)
            # f.create_dataset('orig_ids', data=orig_ids)
            # f.create_dataset('semantic_label', data=semantic_label)
            for key in select_keys:
                f.create_dataset(key, data=node[key], compression="gzip")
        # print(select_keys)
...
def process_model(category, model_id, axis_aligned = False, target_only=False, single_axis_constraint=False, keypoint_based=False):
    ...
    # Collect OBJ mesh files. add key 'objs' to each leaf
    ...
    # Load meshes. add key 'vertices' and 'faces' to each leaf
    partnet_mesh_dir = os.path.join(g_partnet_dir, str(model_id), 'objs')
    leaves = load_meshes(leaves, partnet_mesh_dir)

    # Use axis-aligned bounding box. add key 'box' to each leaf
    if (axis_aligned):
        leaves = compute_aabbox(leaves)

    # Get semantic labels. add key 'semantic_label' to each leaf but not used
    leaves = get_semantics(leaves)

    # Compute parametric models. add key 'default_param' and 'vertices_mat' to each leaf
    leaves = compute_param_models(leaves, target_only = target_only)
    if not leaves: return leaves

    # Sample points. add key 'points' and 'points_mat' to each leaf. add key 'all_points' to each leaf all 2048 points in sum
    ...
    out_all_dir = os.path.join(g_structurenet_output_dir_all, category)

    # save_shape_data(leaves, shape, out_all_dir, model_id, target_only=target_only)
    save_part_data(leaves, shape, out_dir, model_id, target_only=target_only)
...
if __name__ == "__main__":
    ...
    def mkdir(path):
        ...
        os.makedirs(os.path.join(path, args.category), exist_ok=True)
        
        os.makedirs(os.path.join(path, args.category, "rendering_boxes"), exist_ok=True)
        os.makedirs(os.path.join(path, args.category, "rendering_mesh"), exist_ok=True)
        os.makedirs(os.path.join(path, args.category, "rendering_pc"), exist_ok=True)
        os.makedirs(os.path.join(path, args.category, "rendering_both"), exist_ok=True)
        os.makedirs(os.path.join(path, args.category, "h5"), exist_ok=True)
        os.makedirs(os.path.join(path, args.category, "boxes"), exist_ok=True)
        os.makedirs(os.path.join(path, args.category, "mesh"), exist_ok=True)
        os.makedirs(os.path.join(path, args.category, "point_cloud"), exist_ok=True)

    mkdir(g_structurenet_output_dir)
    mkdir(g_structurenet_output_dir_all)
