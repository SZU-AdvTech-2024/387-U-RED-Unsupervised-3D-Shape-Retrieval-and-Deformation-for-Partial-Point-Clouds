...
def get_model(h5_file, semantic=False, mesh=False, constraint=False):
	with h5py.File(h5_file, 'r') as f:
		...
		all_points = f["all_points"][:] 
		all_point_labels = f['all_point_labels'][:]
	...
	if (semantic):
		return box_params, orig_ids, default_param, points, point_labels, points_mat, point_semantic, all_points, all_point_labels
...
def save_dataset(fname, pcs, labels, semantics, model_ids):
	...
	fout.create_dataset('model_id', data=cloud_id, compression='gzip', dtype='str')
	fout.close()
...
def get_targets_h5(target_models, datapath, filename):
	...
	for i in range(len(target_models)):
		...
		if h5_file not in all_files:
			...
			continue
		box_params, orig_ids, default_param, points, point_labels, points_mat, point_semantic, all_points, all_point_labels = get_model(os.path.join(datapath, h5_file), semantic=True)
		
		target_points.append(all_points)
		target_labels.append(all_point_labels)
...
def collect_sources_and_target_splits(source_data_fol, target_data_fol, num_sources, output_filename, source_txt_file=None):
	...
	# all_target_model_ids = list(set([x.split('_')[0] for x in all_target_files]))
	### Use 10% of targets 
	...
	all_source_model_ids = list(set([x.split('_')[0] for x in all_source_files]))
	all_source_model_ids = sorted(all_source_model_ids, key=int)
	for sid in source_models:
	...
	source_models_part = [x for x in all_source_files if os.path.basename(x).split('_')[0] in source_models]
 
	# Remove from list of targets
	for sid in source_models:
		if int(sid) in all_target_model_ids:
			all_target_model_ids.remove(int(sid))
	#####
	...
	print("Num sources part: "+ str(len(source_models_part)))
	print("Num training: "+str(len(train_split)))	
	print("Num test: "+str(len(test_split)))
	print("Total: "+str(len(source_models)+len(train_split)+len(test_split)))

	data_dict = {}
	data_dict["sources"] = source_models
	data_dict["sources_part"] = source_models_part
...
if __name__ == "__main__":
	...
	parser.add_argument('--dump_dir', default= "/mnt/d/20240308/U-RED/data/20240617/generated_datasplits", type=str)
	...
	source_data_fol = os.path.join("/mnt/d/20240308/U-RED/data/20240617/data_aabb_all_models", OBJ_CAT, "h5")
	target_data_fol = os.path.join("/mnt/d/20240308/U-RED/data/20240617/data_all_models", OBJ_CAT, "h5")
