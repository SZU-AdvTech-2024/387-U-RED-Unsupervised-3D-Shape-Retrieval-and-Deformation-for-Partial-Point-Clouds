...
from Density_aware_Chamfer_Distance.utils_v2.model_utils import calc_dcd, calc_cd, calc_emd
...
def read_h5(file):
    with h5py.File(file, 'r') as f:
        pc = normalize_pts(np.array(f['points'][:]))
        pc = torch.tensor(pc).unsqueeze(0).float()
    return pc
...
def compute_cm_loss(source_p, target_p, batch_reduction=None):
    loss, _ = chamfer_distance(source_p, target_p, batch_reduction=batch_reduction)
    return loss

dist = ChamferLoss()
def compute_cm_loss2(p1, p2):
...
emdist = EMDLoss()
def compute_emd_loss(p1, p2):
    return emdist(p1, p2)

def compute_dcd_loss(p1, p2):
    dcd, cd_s, cd_m = calc_dcd(p1, p2)
    return dcd, cd_s, cd_m

def compute_emd_loss2(p1, p2):
    emd_s, _ = calc_emd(p1, p2)
    return emd_s
