...
def regularization_param(params_full, mask_part):
    mask_part = mask_part.reshape(-1).bool()
    params_full = params_full.reshape(-1, 6)
    loss = torch.norm(params_full[mask_part], p=2, dim=-1)
