...
def residual_retrieval_loss(x, x_source, residuals, mask_part=None):
    # matching_m: typically bs x 1024 x 2048
    # x is the input point, bs x numpoints x 3, x_source is bs x 2048 x 3
    # residulas bs x 2048 x 3
    # find the nearest neighbor of x and x_source
    idx = mask_part.sum(1) * 1024
    nn_all = []
    for bs in range(len(x)):
        _, _, nn = ops.knn_points(x[bs:bs+1], x_source[bs:bs+1, :int(idx[bs].item())], K=1, return_nn=True)
        nn = nn.squeeze(2)  # bs x 2048 x 3:the K nearest neighbors in p2 for each point in p1.
        nn_all.append(nn)
    # _, _, nn = ops.knn_points(x, x_source, K=1, return_nn=True)
    nn = torch.cat(nn_all, dim=0)
